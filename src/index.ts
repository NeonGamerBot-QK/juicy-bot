    ...
    await db.user.update({
      ...
      data: {
        ...
        juice_token: newData.token,
            ...
            // use prisma decimal
            juice_hours: new PrismaClient.default.Prisma.Decimal(newData.totalStretchHours),
            ...
            blocks: [
                ...
                },
                {
                    ...
                    element: {
                        ...
                        multiline: true,
                        placeholder: { type: 'plain_text', text: 'Provide a brief description (optional)' }
                    }
                }
            ]
        }
...
slackApp.view('video_submission', async ({ body, context }) => {
    ...
    const view = body.view;
     ...
     const videoDescription = view.state.values.video_description_block.video_description.value || 'No description provided';
...
if(!userEntry.session_started) return;
    ...
    await fetch("https://sww48o88cs88sg8k84g4s4kg.a.selfhosted.hackclub.com/api/video/upload", {
        method: "POST",
        body: form
